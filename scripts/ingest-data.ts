 ...
 * データ取り込みスクリプト
 *
 * Google Sheetsからデータを取得し、クリーニング・チャンク分割・埋め込み生成を行い、
...
import { generateSheetNames, getSheetData, getCategoryFromSheetName } from '../src/lib/google-sheets';
...
async function ingestData(options: IngestionOptions = {}) {
  ...
  console.log('='.repeat(80));
  console.log(`対象シート: M6CH${String(startSheet).padStart(5, '0')} 〜 M6CH${String(endSheet).padStart(5, '0')}`);
  ...
  try {
    ...
    const sheetNames = generateSheetNames(startSheet, endSheet);
